...
export interface IEventInteractionsFirebaseRepository {
    findByEventAndUser(eventId: string, userId: string, sessionId?: string): Promise<FirebaseEventInteractionDto | null>;
    createLikeInteraction(eventId: string, userId: string, liked: boolean, eventData: DenormalizedEventFirebaseData, sessionId?: string): Promise<void>;
